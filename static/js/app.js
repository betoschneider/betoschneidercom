...
function renderPublic(projects) {
  ...
  root.innerHTML = projects.map(p => `
    ...
    ${p.repo_url ? `<div style="margin-top: 5px; font-size: 0.9em;"><a href="${p.repo_url}" target="_blank" style="color: #6c757d;">[Código Fonte]</a></div>` : ''}
  `).join('');
}
...
async function loadProjectsForAdmin() {
  ...
  projects.forEach(p => {
    ...
    div.innerHTML = `
      ...
      ${p.repo_url ? `<p><a href="${p.repo_url}" target="_blank">Repo: ${p.repo_url}</a></p>` : ''}
      <div>
...
async function createProject(token) {
  ...
  const res = await fetch('/projects', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Admin-Token': token }, body: JSON.stringify({ title, description, url, repo_url }) });
...
async function updateProject(token) {
  ...
  const url = document.getElementById('url').value.trim();
  const repo_url = document.getElementById('repo_url').value.trim();
  const res = await fetch(`/projects/${editingId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'X-Admin-Token': token }, body: JSON.stringify({ title, description, url, repo_url }) });
  if (!res.ok) { alert('Erro: ' + res.status); return; }
